...
class ZiyuDataPreProc(object):
    ...
    def __get_means_mode(self, data):
        ...
        """
        # Mean
        summary = data.select(DataChecker.keys_num).describe()
        mean_X = summary.filter(summary.summary == 'mean').rdd.collect()[0].asDict()
        mean_X.pop('summary')
        merged = {}
...
if __name__ == "__main__":
    spark = SparkSession\
        ...
        .getOrCreate()
    model_path = "./model"
    data_all = spark.read.csv(path='./train.csv',encoding='gbk',header=True,inferSchema=True)
    train = data_all
    pre_proc = ZiyuDataPreProc()
    train_proc = pre_proc.data_fit_transform(train)
    rf = RandomForestClassifier(labelCol="label", featuresCol="features", maxDepth=10,numTrees=100,featureSubsetStrategy='0.4',seed=10)
                ...
                for file in csv_files:
                    ...
                    if data_status[0] != 0 and data_status[0] != 3:
                        ...
                        if trans_test == []:
                            ...
                            res = labelConverter.transform(predicter)
                            #res.limit(5).show()
                            res.select(["自恢复状态","prediction","predictedLabel"]).repartition(1).write.csv(path=res_dir+file_name+'.res',header=True)
